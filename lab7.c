...
void get_stats(driver_t driver_list[NRACERS], stats_t *race_stats) 
{
	...
	driver_list[0].tries[0] = race_stats -> slowest_time;
	driver_list[0].tries[0] = race_stats -> fastest_time;
	
	for (d = 0; d < NRACERS; d++) {
		driver_list[d].deviation = 0;
		driver_list[d].tries[0] = driver_list[d].best_time;
...
void get_median(driver_t driver_list[NRACERS], stats_t *race_stats) {
	...
	if ( (NRACERS%2) == 0) {
		race_stats -> median = driver_list[d]
	}
}
